...
class ChatRequest(BaseModel):
    ...
    model: Optional[str] = None  # For future model selection
...
def call_bedrock(conversation: List[Dict], user_message: str, model: str) -> str:
...
def call_openai(conversation: List[Dict], user_message: str, model: str) -> str:
...
async def chat(request: ChatRequest):
    try:
        ...
        if(request.model == 'gpt-40-mini'):
            assistant_response = call_openai(conversation, request.message, request.model)
        else:
            # call Bedrock for response
            assistant_response = call_bedrock(conversation, request.message, request.model)
